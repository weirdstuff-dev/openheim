...
- **Tool execution** — built-in shell, file read/write/edit, directory listing, ripgrep-style search, web fetch, long-term memory (`remember`/`search_memory`/`edit_memory`/`forget`), and subagent-delegation tools. Trait-based, so you can add your own.
...
src/
  ...
  rag/              Long-term memory via tool calls (feature `rag`)
    ...
    tool.rs         remember / search_memory / edit_memory / forget tools
